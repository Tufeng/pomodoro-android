package com.flowcycle.app.service
...
import com.flowcycle.app.MainActivity
import com.flowcycle.app.R
import com.flowcycle.app.data.model.PomodoroConfig
 ...
 * 负责后台保活计时，锁屏不中断
 */
class FlowCycleTimerService : Service() {

    companion object {
        const val CHANNEL_ID = "flowcycle_timer_channel"
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        when (intent?.action) {
            ACTION_START -> {
                val newConfig = getConfig(intent)
                newConfig?.let { config = it }
                intent.getStringExtra(EXTRA_SCENE_NAME)?.let { sceneName = it }
                startTimer()
            }
            ...
            ACTION_UPDATE_CONFIG -> {
                ...
                newConfig?.let {
                    config = it
                    intent.getStringExtra(EXTRA_SCENE_NAME)?.let { name -> sceneName = name }
    ...
    private fun getConfig(intent: Intent): PomodoroConfig? {
    ...
    private fun emitState() {
        timerState.update { current ->
            ServiceTimerState(
                ...
                config = config,
                sceneName = sceneName,
                phaseJustCompleted = current.phaseJustCompleted
            )
        }
    }
    ...
    private fun buildNotification(): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("🎯 $sceneName · $phaseText")
